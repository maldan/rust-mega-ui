...
impl Ui {
    pub fn tree_node(&mut self, id: &str, label: &str, add: impl FnOnce(&mut Self)) -> bool {
        self.tree_node_ex(id, None, label, true, add)
    }

    /// Expandable tree node with a leading icon (e.g. folder).
    pub fn tree_node_icon(
        ...
        self.tree_node_ex(id, Some(icon), label, true, add)
    }

    /// Non-expandable leaf with icon (e.g. file).
    pub fn tree_leaf_icon(&mut self, id: &str, icon: &str, label: &str) {
        self.tree_node_ex(id, Some(icon), label, false, |_| {});
    }
    ...
    fn tree_node_ex(
        ...
        let mut x = rect.min.x + self.s(4.0);
        if expandable {
            let arrow_s = self.s(12.0);
            let arrow_rect = Rect::from_min_size(
                Vec2::new(x, rect.min.y + (height - arrow_s) * 0.5),
                Vec2::splat(arrow_s),
            );
